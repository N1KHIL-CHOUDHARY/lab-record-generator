...
import type { ExperimentFormData } from '@/utils/validators';

export async function createExperiment(subjectId: string, payload: ExperimentFormData) {
  const { data } = await api.post<ApiResponse<Experiment>>(`/experiments/${subjectId}`, payload);
  return data.data;
}

export async function updateExperiment(id: string, payload: Partial<ExperimentFormData>) {
